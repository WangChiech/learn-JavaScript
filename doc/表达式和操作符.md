...
## 操作符
**操作符相关概念**
- 操作数个数：每个操作符都有预期的**操作数个数**
- 操作数类型与结果类型：JavaScript 中每个操作符都有预期的**操作数值类型**，在类型不符合预期时，会触发[**隐式类型转换**]()将操作数转换成预期的值类型
- 操作符副效应：`++`、`--`、`delete`操作符具有副作用，会使用「操作数**左值（lvalue）**」计算并使用计算结果改变该「操作数**左值（lvalue）**」
