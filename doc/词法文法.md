...
ASCII 只能满足英语语言的语言，为了满足国际信息互通的需要，需要有一种字符集能够表示世界上所有语言中涉及的字符，这就催生出了 [Unicode](https://zh.wikipedia.org/zh-hans/Unicode) 字符集的出现。为了一致性，[Unicode](https://zh.wikipedia.org/zh-hans/Unicode) 字符集被设计成 ASCII 的超集。Unicode 将码点区间（U+0000 - U+10FFFF）分为 17 个平面，每个平面包含 $2^{16}$ (65536)个码位（code point），第 0 个平面称为**基本多文种平面**，其码点区间为 `U+0000` - `U+FFFF`，可以基本满足各语言的使用；第 1 - 16 个平面称为辅助平面。
...
JavaScript 程序是用 Unicode 字符集编写的，源文件编码格式通常为 UTF-8，JavaScript 引擎 V8 接收使用 UCS-2 编码/ UTF-16 编码的字符流。
...
|码点|名称|说明|
|--|--|--|
|`u+200F`|right to left mark|从右至左书写标记，可在HTML中表现为`&rlm;` `&#x200F;`或者`&#8207;`|
|`u+200E`|left to right mark|从左至右书写标记，可在HTML中表现为`&lrm; ` `&#x200E;`或者`&#8206;`|
|`U+FEFF`|byte order mark| 位序掩码，表示字节流方向（会被当作空格对待）|
|`U+200C`|zero width non-joiner|没有宽度的不可见连接符，某些少数民族语言会使用到|
|`U+200D`|zero width joiner|没有宽度的不可见连接符，某些少数民族语言会使用到|
